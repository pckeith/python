...
# This script sends our quotes on Mondays.           *
#                                                    *
...
month = now.month
day = now.day
day_of_week = now.weekday() #Note: Result is a number, starting at 0 (Monday)


if (day_of_week  ==  0): # Inspiration emails will go out on Monday!

    #***********************************************************
    # Get the list of 101 quotes, to be sent on Mondays.       *
    #***********************************************************
    script_dir = f"{Path(__file__).resolve().parent}{os.sep}"
    quotes_file = "quotes.txt"
    quotes_file_path = str(Path(script_dir) / quotes_file)
    ...
    #***********************************************************
    # Get the list of emails you'll be sending quotes to,      *
    # and put the list into a data frame.                      *
    #***********************************************************
    quotes_email_list = "quotes_email_list.csv"
    quotes_email_list_path = str(Path(script_dir) / quotes_email_list)
    print(quotes_email_list_path)
    df = pd.read_csv(quotes_email_list_path)
    quotes_email_headers = df.columns.tolist()
    quotes_email_list = df.values.tolist()

    # Print results
    # print("Headers:", quotes_email_headers)
    # print("Data:", quotes_email_list)

    for receiver_name, receiver_email in quotes_email_list:
        print(f"Name: {receiver_name} | Email: {receiver_email}")

        random_number = random.randint(0, 101)
        selected_quote = quotes[random_number]

        msg = EmailMessage()
        msg["Subject"] = "Inspirational Email for Monday"
        msg["From"] = SENDER_EMAIL
        msg["To"] = receiver_email
        msg.set_content(f"Dear {receiver_name},\n\n{selected_quote}")
